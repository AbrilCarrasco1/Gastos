...
def user_input_features():
  # Entrada
  Presupuesto = st.number_input('Presupuesto:', min_value=0, max_value=10000, value = 0)
  Tiempo = st.number_input('Duracion de actividad',  min_value=0, max_value=10000, value = 0)
  ...
  Momento = st.number_input('Momento del día 1 Mañana 2 Tarde 3 Noche:', min_value=1, max_value=3, value = 1, step = 1)
  Personas = st.number_input('Personas invlolucradas en el gasto', min_value=1, max_value=50, value=1, step = 1)
...
datos =  pd.read_csv('MISGASTOS_df.csv', encoding='latin-1')
